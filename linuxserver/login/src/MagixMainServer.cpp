				...
				case ID_TOKENCONNECTED:
					{
						...
						if(tToken==0)
						{
							...
							char tBuffer[128] = "";
							strcpy(tBuffer,"Failed to assign token for ");
							outFile.write(tBuffer,strlen(tBuffer));
							strcpy(tBuffer,tAdd.ToString());
							outFile.write(tBuffer,strlen(tBuffer));
							strcpy(tBuffer,"\n");
							outFile.write(tBuffer,strlen(tBuffer));
							outFile.close();
				...
				case ID_LOGON:
					{
						...
						if(tLogonSuccess)
						{
							...
							if(outFile.good())
							{
								...
								char *resulttime;
								resulttime = (char*)malloc(26*sizeof(char));
								timeinfo = localtime_r(&rawtime, &timeinfoBuffer);
								//resulttime = asctime_r(timeinfo, resulttime);
								//strcpy(tBuffer,resulttime);
								char tBuffer[128] = "";
				...
				case ID_VIOLATION:
					{
						...
						char tBuffer[128] = "";
						outFile.write(tInfo,strlen(tInfo));
						strcpy(tBuffer," -> ");
						outFile.write(tBuffer,strlen(tBuffer));
						strcpy(tBuffer,tAdd.ToString());
						outFile.write(tBuffer,strlen(tBuffer));
						if(tToken>0 && tToken<=MAX_CLIENTS)
						{
							strcpy(tBuffer," -> ");
							outFile.write(tBuffer,strlen(tBuffer));
							strcpy(tBuffer,playerToken[tToken-1].name.c_str());
							outFile.write(tBuffer,strlen(tBuffer));
							strcpy(tBuffer," [");
							outFile.write(tBuffer,strlen(tBuffer));
							strcpy(tBuffer,"");
							sin << (int)playerToken[tToken-1].charID;
							std::string val = sin.str();
							strcpy(tBuffer,val.c_str());
							//tBuffer += val;
							//itoa((int)playerToken[tToken-1].charID,tBuffer,10);
							outFile.write(tBuffer,strlen(tBuffer));
							strcpy(tBuffer,"]");
							outFile.write(tBuffer,strlen(tBuffer));
						}
	...
	void sendServerList(const SystemAddress &target, const bool &isClient)
	{
		...
		for(int i=0; i<MAX_SERVERS; i++)
		{
			if(serverTunnelAdd[i]!=UNASSIGNED_SYSTEM_ADDRESS)
			{
				...
				if(target.binaryAddress==2130706433)
					if(isClient)tBitStream.Write(2130706433);
					else tBitStream.Write(2130706433);
				else 
				{
